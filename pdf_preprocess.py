...
generator = pipeline("text-generation", model="Groq/Llama-3.2")  # Replace with your preferred model
...
def load_faiss_index(index_path, metadata_path):
    ...
    metadata = np.load(metadata_path, allow_pickle=True)["metadata"].tolist()
...
def query_bot(query, index, metadata, embedding_model, top_k=5):
    """
    ...
    Args:
        ...
        metadata (list): List of metadata (e.g., document texts or names).
...
if __name__ == "__main__":
    ...
    metadata_path = r"D:\RTI Judgments\metadata.npz"
    ...
    while True:
        ...
        response = query_bot(user_query, faiss_index, metadata, embedding_model)
        print("\nResponse:\n", response)
